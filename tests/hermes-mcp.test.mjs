...
test("Hermes MCP tools bind the only authorized coach server-side", () => {
  ...
  assert.doesNotMatch(
    source,
    /def (?:get_member_by_id|list_customer_ids|create_member_binding_qr|bind_member_external_userid|create_member_message_draft|create_member_message|confirm_customer_send_task|get_customer_send_task_status)\([^)]*coach_userid/s,
  );
  assert.match(source, /def create_member_binding_qr\(member_id: str\)/);
  assert.match(source, /禁止按昵称匹配/);
  assert.match(source, /def create_member_message\(/);
});
