...
test("Hermes MCP tools bind the only authorized coach server-side", () => {
  ...
  assert.doesNotMatch(
    source,
    /def (?:get_member_by_id|list_customer_ids|bind_member_external_userid|create_member_message_draft|confirm_customer_send_task|get_customer_send_task_status)\([^)]*coach_userid/s,
  );
});
